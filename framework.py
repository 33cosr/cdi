...
para = common.parameter.Parameter()
...
file_data = {}
for f in file_list:
    file_data[f] = common.db_connector.get_meta(f)

print file_data

# Check rules
for f in file_data:
    data_set = common.utility.file_to_array(os.path.join(para.landing_dir, f))
    data_set = [line.split(file_data[f]['delimiter']) for line in data_set]
    cleaner.clean(data_set, file_data[f])
    print 'after clean'
    print data_set
    common.utility.array_to_file(os.path.join(para.staging_dir, f), data_set, file_data[f]['delimiter'])
    # Move to archive
